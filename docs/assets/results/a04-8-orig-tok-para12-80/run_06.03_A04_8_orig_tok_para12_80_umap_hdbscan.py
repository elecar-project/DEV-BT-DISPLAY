...
OUTPUT_DIR = ROOT / "Result/06.03_A04_min-test/A04-8(orig)_tok(para12-80)"
REPORT_PATH = OUTPUT_DIR / "Result_06.03_A04-8(orig)_tok(para12-80).md"
...
def load_a03_comparison() -> pd.DataFrame:
    path = ROOT / "Result/06.03_A03_min-test/min_cluster_charts/A03_min_cluster_chart_summary.csv"
    if not path.exists():
        return pd.DataFrame()
...
def write_report(
    ...
    a03 = load_a03_comparison()
    a03_focus = a03[a03["dataset"].isin(["A03-3(del)_tok(para12-80)", "A03-6(repl)_tok(para12-80)", "A03-8(orig)_tok(para12-80)"])].copy() if not a03.empty else pd.DataFrame()
    ...
    if not best_balance.empty:
        ...
        better_than_a03_8 = (
            int(bb["n_clusters"]) > 2
            and float(bb["largest_topic_ratio"]) < (10308 / 11136)
        )
        comparison_notes.append(
            "A04-8 最佳平衡設定相較 A03-8 有改善：有效主題數高於 2，且最大主題比例低於 A03-8 的 0.9256。"
            if better_than_a03_8
            else "A04-8 最佳平衡設定未完全超越 A03-8：仍需從 noise、主題數或最大主題比例中取捨。"
        )
        comparison_notes.append(
            f"最佳平衡設定為 n_clusters={int(bb['n_clusters'])}, noise_ratio={float(bb['noise_ratio']):.4f}, "
    ...
    strategy_note = (
        ...
        "理由是 A04-8 證明 orig 在調參後可以取得比 A03-8 更好的主題平衡，但 topic words 仍保留明顯品牌/車款錨點；"
    ...
    report = [
        ...
        *[f"- {note}" for note in comparison_notes],
        ...
        "### A03 para12-80 基準摘要",
        "",
        md_table(a03_focus.to_dict("records"), list(a03_focus.columns)) if not a03_focus.empty else "未找到 A03 summary CSV。\n",
        "### 品牌/車款詞彙檢查",
        "",
