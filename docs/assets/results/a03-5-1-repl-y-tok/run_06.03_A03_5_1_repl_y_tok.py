...
DATASET_DIR = ROOT / "Result/06.03_A02/R06.03_A02-pre_LLM(repl)_tok_dataset"
OUTPUT_DIR = ROOT / "Result/06.03_A03_min-test/A03-5.1(repl-y)_tok"

REPORT_PATH = OUTPUT_DIR / "Result_06.03_A03-5.1(repl-y)_tok.md"
MIN_TEST_CSV = OUTPUT_DIR / "Result_06.03_A03-5.1(repl-y)_tok-min_cluster_size.csv"
BEST_TOPIC_INFO_CSV = OUTPUT_DIR / "Result_06.03_A03-5.1(repl-y)_tok-best_topic_info.csv"
BEST_DOC_TOPICS_CSV = OUTPUT_DIR / "Result_06.03_A03-5.1(repl-y)_tok-best_document_topics.csv"
RUN_LOG_JSON = OUTPUT_DIR / "Result_06.03_A03-5.1(repl-y)_tok-run_log.json"
EMBEDDINGS_CACHE = OUTPUT_DIR / "embeddings_all-MiniLM-L6-v2.npy"
UMAP_CACHE = OUTPUT_DIR / "umap_n15_c5_d0_cosine_rs42.npy"

CUSTOM_STOP_WORDS_EXTRA = ["brand", "model"]
CUSTOM_STOP_WORDS = sorted(set(ENGLISH_STOP_WORDS).union(CUSTOM_STOP_WORDS_EXTRA))
...
def count_labels(labels: np.ndarray) -> dict[str, Any]:
    raw_counts = Counter(int(label) for label in labels)
    ordered_topics = sorted(
        [topic for topic in raw_counts if topic != -1],
        key=lambda topic: (-raw_counts[topic], topic),
    )
    counts = {-1: raw_counts.get(-1, 0)}
    counts.update({new_topic: raw_counts[old_topic] for new_topic, old_topic in enumerate(ordered_topics)})
    n_clusters = len(ordered_topics)
    return {
...
def train_best_topic_model(
    ...
    topic_model = BERTopic(
        ...
        vectorizer_model=CountVectorizer(stop_words=CUSTOM_STOP_WORDS, ngram_range=(1, 2), min_df=2),
        top_n_words=10,
...
def run() -> None:
    ...
    errors.append(
        {
            "流程": "BERTopic 停用詞",
            "問題": "CountVectorizer custom stop_words 已加入 brand、model；此設定作用於 c-TF-IDF/topic words 階段，不改動原始文本與 embeddings。",
        }
    )
    ...
    try:
        ...
        if EMBEDDINGS_CACHE.exists():
            ...
            if len(embeddings) != len(documents):
                errors.append({"流程": "Embedding", "問題": "既有 embeddings 快取筆數與 documents 不一致，已重新計算。"})
            ...
            embeddings = embedding_model.encode(documents, show_progress_bar=True, batch_size=64)
            np.save(EMBEDDINGS_CACHE, embeddings)
            errors.append({"流程": "Embedding", "問題": "已完成 embeddings 計算並寫入快取。"})
    except Exception:
    ...
    try:
        if UMAP_CACHE.exists():
            ...
            if len(reduced_embeddings) != len(documents):
                errors.append({"流程": "UMAP", "問題": "既有 UMAP 快取筆數與 documents 不一致，已重新計算。"})
                ...
                errors.append({"流程": "UMAP", "問題": "偵測到既有 UMAP 快取，已直接共用。"})
        else:
            reduced_embeddings = UMAP(
                n_neighbors=15,
                n_components=5,
            ...
            np.save(UMAP_CACHE, reduced_embeddings)
            errors.append({"流程": "UMAP", "問題": "已完成 UMAP 降維並寫入快取。"})
    except Exception:
        ...
        json.dumps(
            {
                ...
                "dataset_meta": meta,
                "custom_stop_words_extra": CUSTOM_STOP_WORDS_EXTRA,
                "rows": rows,
                "errors": errors,
                "best": best_payload,
    ...
    param_rows = [
        ...
        {"參數": "HDBSCAN", "設定": "metric=euclidean, cluster_selection_method=eom, prediction_data=True", "統計/原因": "對固定 UMAP 結果測試 min_cluster_size；topic count 依 BERTopic 風格由大到小重編號"},
        {"參數": "CountVectorizer", "設定": "custom stop_words=english + brand + model, ngram_range=(1, 2), min_df=2", "統計/原因": "最佳參數 BERTopic 訓練時使用，並在 c-TF-IDF/topic words 階段移除 brand、model"},
