...
OUTPUT_DIR = ROOT / "Result/M02_single/06.08_Test1-1(del-new-tp-X)"
A04_EMBEDDINGS = ROOT / "Result/06.03_A04_min-test/A04-3(del)_tok(para12-80)/cache/embeddings_all-MiniLM-L6-v2.npy"
A04_EMBEDDINGS_META = ROOT / "Result/06.03_A04_min-test/A04-3(del)_tok(para12-80)/cache/embeddings_all-MiniLM-L6-v2.meta.json"
...
RUN_BASENAME = "06.08_Test1-1(del-new-tp-X)"


CONFIGS = [
    {
        "selection_label": "06.08_Test1-1(del-new-tp-X)",
        "report_name": f"{RUN_BASENAME}.md",
        "selection_reason": "使用者指定 del-new-tp-X 單組參數，並以 BERTopic nr_topics='auto' 做 c-TF-IDF topic reduction。",
...
def get_embeddings(docs: list[str]) -> tuple[SentenceTransformer, np.ndarray, str]:
    ...
    if A04_EMBEDDINGS.exists() and A04_EMBEDDINGS_META.exists():
        ...
        if old_meta == current_meta:
            ...
            return model, np.load(EMBEDDINGS_PATH), f"重用 A04-3(del) embeddings 快取並複製到 {RUN_BASENAME}。"
