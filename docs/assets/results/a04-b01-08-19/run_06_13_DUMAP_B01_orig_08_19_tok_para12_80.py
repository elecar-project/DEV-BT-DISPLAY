...
OUTPUT_DIR = ROOT / "Result/06.13_[D]UMAP/[B]01-(orig)_08-19_tok(para12-80)"
REPORT_PATH = OUTPUT_DIR / "Result_[D]01-(orig)_08-19_tok(para12-80).md"
...
CMIN_DIR = ROOT / "Result/06.13_[C]min/[B]01-(orig)_08-19(240)_tok(para12-80)"
CMIN_TOPIC_INFO_CSV = CMIN_DIR / "Result_06.13_[C]min_[B]01-(orig)_08-19(240)_tok(para12-80)-best_topic_info.csv"
CMIN_MIN_TEST_CSV = CMIN_DIR / "Result_06.13_[C]min_[B]01-(orig)_08-19(240)_tok(para12-80)-min_cluster_size.csv"
...
def label_metrics(labels: np.ndarray) -> dict[str, Any]:
    ...
    return {
        ...
        "topic_0_count": int(cluster_counts_sorted[0]) if len(cluster_counts_sorted) >= 1 else 0,
        "topic_1_count": int(cluster_counts_sorted[1]) if len(cluster_counts_sorted) >= 2 else 0,
...
def compute_balance_score(n_clusters: int, noise_ratio: float, largest_topic_ratio: float, top3_topic_ratio: float) -> float:
    return float(
        0.30 * (1.0 - noise_ratio)
        + 0.30 * (1.0 - largest_topic_ratio)
        + 0.20 * (1.0 - top3_topic_ratio)
        + 0.20 * min(n_clusters / 25.0, 1.0)
    )


def load_cmin_baseline() -> pd.DataFrame:
    if not CMIN_TOPIC_INFO_CSV.exists():
        return pd.DataFrame()
    info = pd.read_csv(CMIN_TOPIC_INFO_CSV)
    if "Topic" not in info.columns or "Count" not in info.columns:
        return pd.DataFrame()
    total = int(info["Count"].sum())
    noise_count = int(info.loc[info["Topic"].eq(-1), "Count"].sum())
    non_noise = info[info["Topic"].ne(-1)].sort_values("Count", ascending=False).copy()
    cluster_counts = [int(value) for value in non_noise["Count"].tolist()]
    n_clusters = len(cluster_counts)
    largest = cluster_counts[0] if cluster_counts else 0
    top3 = sum(cluster_counts[:3]) if cluster_counts else 0
    if n_clusters <= 1 or sum(cluster_counts) == 0:
        entropy = 0.0
    else:
        probs = np.array(cluster_counts, dtype=float) / float(sum(cluster_counts))
    ...
    largest_topic_ratio = float(largest / total) if total else 0.0
    top3_topic_ratio = float(top3 / total) if total else 0.0
    min_cluster_size = None
    min_samples = None
    if CMIN_MIN_TEST_CSV.exists():
        min_df = pd.read_csv(CMIN_MIN_TEST_CSV)
        ok = min_df[min_df["status"].eq("ok") & min_df["n_clusters"].ge(2)].copy()
        if not ok.empty:
            best = ok.sort_values(["noise_ratio", "n_clusters"], ascending=[True, False]).iloc[0]
            min_cluster_size = int(best["min_cluster_size"])
    row = {
        "baseline": "[C]min best",
        "source": str(CMIN_TOPIC_INFO_CSV.relative_to(ROOT)),
        "umap_n_neighbors": 15,
        "umap_n_components": 5,
        "umap_min_dist": 0.0,
        ...
        "n_clusters": int(n_clusters),
        "noise_ratio": noise_ratio,
        "topic_-1_count": noise_count,
        "topic_0_count": int(cluster_counts[0]) if len(cluster_counts) >= 1 else 0,
        "topic_1_count": int(cluster_counts[1]) if len(cluster_counts) >= 2 else 0,
        "largest_topic_count": int(largest),
        "largest_topic_ratio": largest_topic_ratio,
        "top3_topic_ratio": top3_topic_ratio,
        "topic_entropy": entropy,
        "balance_score": compute_balance_score(n_clusters, noise_ratio, largest_topic_ratio, top3_topic_ratio),
    }
...
def write_report(
    ...
    cmin_baseline = load_cmin_baseline()
    ...
    comparison_notes = []
    if not best_balance.empty and not cmin_baseline.empty:
        bb = best_balance.iloc[0]
        base = cmin_baseline.iloc[0]
        better_balance = float(bb["balance_score"]) > float(base["balance_score"])
        lower_largest = float(bb["largest_topic_ratio"]) < float(base["largest_topic_ratio"])
        lower_top3 = float(bb["top3_topic_ratio"]) < float(base["top3_topic_ratio"])
        comparison_notes.append(
            "是否比 [C]min 產生更平衡的 noise 與主題數："
            f"{'是' if better_balance else '否'}。"
            f"[D] best_balance 的 balance_score={float(bb['balance_score']):.4f}，"
            f"[C]min baseline={float(base['balance_score']):.4f}；"
            f"n_clusters {int(base['n_clusters'])} -> {int(bb['n_clusters'])}，"
            f"noise_ratio {float(base['noise_ratio']):.4f} -> {float(bb['noise_ratio']):.4f}。"
        )
        comparison_notes.append(
            "是否降低最大主題集中度："
            f"{'是' if lower_largest else '否'}。"
            f"largest_topic_ratio {float(base['largest_topic_ratio']):.4f} -> {float(bb['largest_topic_ratio']):.4f}；"
            f"top3_topic_ratio {float(base['top3_topic_ratio']):.4f} -> {float(bb['top3_topic_ratio']):.4f}"
            f"{'，前三主題集中度也降低。' if lower_top3 else '。'}"
        )
        comparison_notes.append(
            f"最佳平衡設定為 UMAP(n_neighbors={int(bb['umap_n_neighbors'])}, "
            f"n_components={int(bb['umap_n_components'])}, min_dist={float(bb['umap_min_dist']):.2f})，"
            f"HDBSCAN(min_cluster_size={int(bb['hdbscan_min_cluster_size'])}, "
            f"min_samples={'None' if pd.isna(bb['hdbscan_min_samples']) else int(bb['hdbscan_min_samples'])}, "
            f"method={bb['hdbscan_cluster_selection_method']}, "
            f"epsilon={float(bb['hdbscan_cluster_selection_epsilon']):.2f})。"
        )
    elif not best_balance.empty:
        bb = best_balance.iloc[0]
        comparison_notes.append(
            "找不到 [C]min baseline topic_info.csv，因此只列出本次最佳平衡設定："
            f"n_clusters={int(bb['n_clusters'])}, noise_ratio={float(bb['noise_ratio']):.4f}, "
            f"largest_topic_ratio={float(bb['largest_topic_ratio']):.4f}, top3_topic_ratio={float(bb['top3_topic_ratio']):.4f}。"
        )
    strategy_note = (
        "建議以 repl 作為主要語料策略，del 作為敏感性/驗證版本，orig 作為對照與品牌/車款專題分析。"
        "理由是本次只使用 orig 已能透過 UMAP/HDBSCAN 調參改善最大主題集中度，但 topic words 仍可能保留品牌/車款錨點；"
    ...
    report = [
        ...
        *[f"- {note}" for note in comparison_notes],
        ...
        "### [C]min 基準摘要",
        "",
        md_table(cmin_baseline.to_dict("records"), list(cmin_baseline.columns)) if not cmin_baseline.empty else "未找到 [C]min baseline CSV。\n",
        "### 品牌/車款詞彙檢查",
        "",
...
def run() -> None:
    ...
    errors: list[dict[str, str]] = [{"流程": "環境開始運行", "問題": "環境套件可載入，開始 [D] UMAP + HDBSCAN 多參數檢測。"}]
    full_failures: list[str] = []
    try:
