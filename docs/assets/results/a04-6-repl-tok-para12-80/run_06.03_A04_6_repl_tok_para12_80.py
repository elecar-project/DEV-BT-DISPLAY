...
OUTPUT_DIR = ROOT / "Result/06.03_A04_min-test/A04-6(repl)_tok(para12-80)"
REPORT_PATH = OUTPUT_DIR / "Result_06.03_A04-6(repl)_tok(para12-80).md"
STAGE1_CSV = OUTPUT_DIR / "Result_06.03_A04-6(repl)_tok(para12-80)-stage1_umap_hdbscan.csv"
STAGE2_CSV = OUTPUT_DIR / "Result_06.03_A04-6(repl)_tok(para12-80)-stage2_hdbscan_deep.csv"
SELECTED_CSV = OUTPUT_DIR / "selected_configs.csv"
STABILITY_CSV = OUTPUT_DIR / "Result_06.03_A04-6(repl)_tok(para12-80)-top10_stability.csv"
ALL_RESULTS_CSV = OUTPUT_DIR / "Result_06.03_A04-6(repl)_tok(para12-80)-all_results.csv"
...
COMBINED_TOPIC_INFO_CSV = OUTPUT_DIR / "topic_info.csv"
COMBINED_DOCUMENT_TOPICS_CSV = OUTPUT_DIR / "document_topics.csv"
COMBINED_REPRESENTATIVE_DOCS_CSV = OUTPUT_DIR / "representative_docs.csv"
COMBINED_TOPIC_WORDS_CSV = OUTPUT_DIR / "topic_words.csv"
...
def select_best(stage2: pd.DataFrame) -> pd.DataFrame:
    ...
    if not strict.empty:
        balance_pool = strict
        balance_reason = "符合原始平衡條件後取最高 balance_score。"
...
def train_final_models(
    ...
    for _, row in selected.iterrows():
        ...
        try:
            ...
            topic_info_path = out / "topic_info.csv"
            document_topics_path = out / "document_topics.csv"
            topic_words_path = out / "topic_words.csv"
            representative_docs_path = out / "representative_docs.csv"
            size_distribution_path = out / "topic_size_distribution.csv"
            config_path = out / "final_config.json"
            info.to_csv(topic_info_path, index=False)
            pd.DataFrame({"title": titles, "sentence": docs, "topic": [int(t) for t in topics]}).to_csv(
                document_topics_path, index=False
            )
            topic_words = []
            representative_docs = []
            ...
            pd.DataFrame(topic_words).to_csv(topic_words_path, index=False)
            pd.DataFrame(representative_docs).to_csv(representative_docs_path, index=False)
            size_dist = info[["Topic", "Count", "Name"]].copy()
            size_dist["ratio"] = size_dist["Count"] / len(docs)
            size_dist.to_csv(size_distribution_path, index=False)
            final_config = {
                ...
                "search_metrics": {
                    ...
                    "topic_-1_count": int(row["topic_-1_count"]),
                    "topic_0_count": int(row["topic_0_count"]),
                    "topic_1_count": int(row["topic_1_count"]),
                    "largest_topic_count": int(row["largest_topic_count"]),
            ...
            summaries.append(
                {
                    ...
                    "status": "ok",
                    "output_dir": rel(out),
                    "topic_info": rel(topic_info_path),
                    "document_topics": rel(document_topics_path),
                    "representative_docs": rel(representative_docs_path),
                    "topic_words": rel(topic_words_path),
                    "topic_size_distribution": rel(size_distribution_path),
                    "final_config": rel(config_path),
                    "n_clusters": int(row["n_clusters"]),
                    "noise_ratio": float(row["noise_ratio"]),
                    "largest_topic_ratio": float(row["largest_topic_ratio"]),
                    "top3_topic_ratio": float(row["top3_topic_ratio"]),
...
def export_combined_final_outputs(final_summary: list[dict[str, Any]]) -> None:
    combined_targets = [
        ("topic_info", COMBINED_TOPIC_INFO_CSV),
        ("document_topics", COMBINED_DOCUMENT_TOPICS_CSV),
        ("representative_docs", COMBINED_REPRESENTATIVE_DOCS_CSV),
        ("topic_words", COMBINED_TOPIC_WORDS_CSV),
    ]
    for key, target in combined_targets:
        frames = []
        for item in final_summary:
            rel_path = item.get(key)
            if item.get("status") != "ok" or not rel_path:
                continue
            source = OUTPUT_DIR / str(rel_path)
            if not source.exists():
                continue
            frame = pd.read_csv(source)
            frame.insert(0, "selection_label", item.get("selection_label", ""))
            frames.append(frame)
        if frames:
...
def plot_largest_topic_ratio_comparison(df: pd.DataFrame) -> None:
    ok = df[df["status"].eq("ok")].copy()
    ok["config"] = (
        "nn" + ok["umap_n_neighbors"].astype(int).astype(str)
        + "_nc" + ok["umap_n_components"].astype(int).astype(str)
        + "_md" + ok["umap_min_dist"].astype(str)
        + "_mcs" + ok["hdbscan_min_cluster_size"].astype(int).astype(str)
        + "_ms" + ok["hdbscan_min_samples"].astype(object).where(ok["hdbscan_min_samples"].notna(), "None").astype(str)
        + "_" + ok["hdbscan_cluster_selection_method"].astype(str)
        + "_eps" + ok["hdbscan_cluster_selection_epsilon"].astype(str)
    )
    plot_df = ok.sort_values(["largest_topic_ratio", "noise_ratio", "n_clusters"], ascending=[True, True, False]).head(80)
    fig, ax = plt.subplots(figsize=(18, 6))
    ax.bar(range(len(plot_df)), plot_df["largest_topic_ratio"], color="#536a8f")
    ax.axhline(0.60, color="#c44e52", linestyle="--", linewidth=1, label="target 0.60")
    ax.set_xticks(range(len(plot_df)))
    ax.set_xticklabels(plot_df["config"], rotation=90, fontsize=6)
    ax.set_xlabel("top 80 parameter combinations by lowest largest_topic_ratio")
    ax.set_ylabel("largest_topic_ratio")
    ax.set_title("largest_topic_ratio comparison across parameter combinations")
    ax.legend()
    fig.tight_layout()
    fig.savefig(CHART_DIR / "largest_topic_ratio_parameter_comparison.png", dpi=180)
    plt.close(fig)
...
def plot_best_three_topic_distributions(final_summary: list[dict[str, Any]]) -> None:
    ok_items = [row for row in final_summary if row.get("status") == "ok" and row.get("topic_size_distribution")]
    if not ok_items:
        return
    fig, axes = plt.subplots(len(ok_items), 1, figsize=(14, 4 * len(ok_items)), squeeze=False)
    for ax, item in zip(axes[:, 0], ok_items):
        dist = pd.read_csv(OUTPUT_DIR / item["topic_size_distribution"])
        dist = dist.sort_values("Count", ascending=False).head(35)
        ax.bar(dist["Topic"].astype(str), dist["Count"], color="#4a8f73")
        ax.set_title(f"Topic size distribution - {item['selection_label']}")
        ax.set_xlabel("topic")
        ax.set_ylabel("count")
        ax.tick_params(axis="x", rotation=45)
    fig.tight_layout()
...
def create_charts(stage2: pd.DataFrame, selected: pd.DataFrame, final_summary: list[dict[str, Any]], errors: list[dict[str, str]]) -> list[str]:
    ...
    funcs = [
        ...
        ("largest_topic_ratio_parameter_comparison.png", lambda: plot_largest_topic_ratio_comparison(stage2)),
...
def write_report(
    ...
    lowest = selected[selected["selection_label"].eq("lowest_noise")].iloc[0] if "selection_label" in selected.columns and selected["selection_label"].eq("lowest_noise").any() else None
    best_balance = selected[selected["selection_label"].eq("best_balance")].iloc[0] if "selection_label" in selected.columns and selected["selection_label"].eq("best_balance").any() else None
    if best_balance is not None:
        improved = bool(
            best_balance["n_clusters"] >= 4
            and best_balance["noise_ratio"] <= 0.35
            and best_balance["largest_topic_ratio"] <= 0.60
            and best_balance["top3_topic_ratio"] <= 0.85
        )
        usable_para = bool(best_balance["n_clusters"] >= 4 and best_balance["largest_topic_ratio"] <= 0.60)
    else:
        improved = False
        usable_para = False
    if lowest is not None:
        lowest_unusable = bool(lowest["n_clusters"] < 4 or lowest["largest_topic_ratio"] > 0.60 or lowest["top3_topic_ratio"] > 0.85)
    else:
        lowest_unusable = True
    err_rows = []
    ...
    report = [
        ...
        "## 0. 研究結論",
        "",
        f"- 是否改善 topic 過度集中：{'是' if improved else '未完全改善'}。判斷依據為最佳平衡設定是否同時滿足 n_clusters >= 4、noise_ratio <= 0.35、largest_topic_ratio <= 0.60、top3_topic_ratio <= 0.85。",
        f"- 最低 noise 設定是否仍然不可用：{'是，最低 noise 設定仍有過度集中或主題數不足風險' if lowest_unusable else '否，最低 noise 設定也通過主要平衡門檻'}。",
        "- 最佳平衡設定："
        + (
            f"UMAP(n_neighbors={int(best_balance['umap_n_neighbors'])}, n_components={int(best_balance['umap_n_components'])}, min_dist={float(best_balance['umap_min_dist'])}); "
            f"HDBSCAN(min_cluster_size={int(best_balance['hdbscan_min_cluster_size'])}, min_samples={best_balance['hdbscan_min_samples']}, "
            f"method={best_balance['hdbscan_cluster_selection_method']}, epsilon={float(best_balance['hdbscan_cluster_selection_epsilon'])}); "
            f"n_clusters={int(best_balance['n_clusters'])}, noise_ratio={float(best_balance['noise_ratio']):.4f}, "
            f"largest_topic_ratio={float(best_balance['largest_topic_ratio']):.4f}, top3_topic_ratio={float(best_balance['top3_topic_ratio']):.4f}, "
            f"balance_score={float(best_balance['balance_score']):.4f}。"
            if best_balance is not None
            else "尚未選出。"
        ),
        f"- 是否建議使用 para12-80 作為正式研究語料：{'建議，可進入正式語意檢查與人工命名' if usable_para else '暫不建議直接作為唯一正式語料，建議先檢查最大主題內容或比較其他切分版本'}。",
        "",
        "## 1. Dataset 資訊",
        "",
    ...
    report.extend(
        [
            ...
            f"- combined topic_info.csv: `{rel(COMBINED_TOPIC_INFO_CSV)}`",
            f"- combined document_topics.csv: `{rel(COMBINED_DOCUMENT_TOPICS_CSV)}`",
            f"- combined representative_docs.csv: `{rel(COMBINED_REPRESENTATIVE_DOCS_CSV)}`",
            f"- combined topic_words.csv: `{rel(COMBINED_TOPIC_WORDS_CSV)}`",
            f"- all_results.csv: `{rel(ALL_RESULTS_CSV)}`",
            "",
            "## 11. 穩定性檢測摘要",
            "",
...
def run() -> None:
    ...
    try:
        ...
        pd.DataFrame(final_summary).to_csv(COMPARISON_SUMMARY_CSV, index=False)
        export_combined_final_outputs(final_summary)
        final_configs = []
        for item in final_summary:
            config_rel = item.get("final_config")
            if item.get("status") == "ok" and config_rel:
                final_configs.append(json.loads((OUTPUT_DIR / config_rel).read_text(encoding="utf-8")))
            else:
