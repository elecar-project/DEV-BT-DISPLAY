...
OUTPUT_DIR = ROOT / "Result/06.06_M02_single/M02-1(orig-new-tp-30)"
...
RUN_BASENAME = "M02-1(orig-new-tp-30)"


CONFIGS = [
    {
        "selection_label": "M02-1(orig-new-tp-30)",
...
def get_embeddings(docs: list[str]) -> tuple[SentenceTransformer, np.ndarray, str]:
    ...
    if EMBEDDINGS_PATH.exists() and EMBEDDINGS_META.exists():
        ...
        if old_meta == current_meta:
            return model, np.load(EMBEDDINGS_PATH), "使用 M02-1(orig-new-tp-30) 既有 embeddings 快取。"
    if A04_EMBEDDINGS.exists() and A04_EMBEDDINGS_META.exists():
        ...
        if old_meta == current_meta:
            ...
            return model, np.load(EMBEDDINGS_PATH), "重用 A04-8(orig) embeddings 快取並複製到 M02-1(orig-new-tp-30)。"
    embeddings = model.encode(docs, show_progress_bar=True, batch_size=64, convert_to_numpy=True)
    np.save(EMBEDDINGS_PATH, embeddings)
    EMBEDDINGS_META.write_text(json.dumps(current_meta, ensure_ascii=False, indent=2), encoding="utf-8")
    return model, embeddings, "重新計算 embeddings 並建立 M02-1(orig-new-tp-30) 快取。"
...
def write_report(
    ...
    report = [
        f"# M02-1(orig-new-tp-30) BERTopic + A05-8.4(human) stopwords + c-TF-IDF reduction - {label}",
        "",
        "## Run Info",
        "",
...
def main() -> None:
    ...
    (OUTPUT_DIR / "M02-1(orig-new-tp-30)_custom_stopwords_used.txt").write_text(
    ...
    pd.DataFrame(summaries).to_csv(OUTPUT_DIR / "M02-1(orig-new-tp-30)_summary.csv", index=False)
    (OUTPUT_DIR / "M02-1(orig-new-tp-30)_run_summary.json").write_text(
        json.dumps(summaries, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    (OUTPUT_DIR / "M02-1(orig-new-tp-30)_run_log.json").write_text(
        json.dumps(
            {
