...
DATASET_DIR = ROOT / "Result/06.03_A02/R06.03_A02-pre_LLM(del)_tok_dataset"
OUTPUT_DIR = ROOT / "Result/06.03_A04_min-test/A04-2(del)_tok"
REPORT_PATH = OUTPUT_DIR / "Result_06.03_A04-2(del)_tok.md"
...
def run_hdbscan_grid(
    ...
    for uidx, uconf in enumerate(umap_configs, start=1):
        ...
        expected_keys = {
            (nn, nc, md, seed, min_cluster_size, min_samples, method, epsilon)
        ...
        if expected_keys.issubset(done):
            ...
            continue
        if max_new_umaps and new_umaps_processed >= max_new_umaps:
            break
        try:
...
def run() -> None:
    ...
    max_new_umaps = int(os.environ.get("A04_MAX_NEW_UMAPS", "0") or "0")
    ...
    try:
                ...
                json.dumps(
                    {
                        ...
                        "expected_stage1_rows": expected_stage1_rows,
                        "note": "Stage 1 checkpoint incomplete; rerun script to continue.",
                    },
                    ensure_ascii=False,
                    indent=2,
                ),
        ...
        expected_stage2_rows = len(stage2_umaps) * len(STAGE2_MIN_CLUSTER_SIZE) * len(STAGE2_MIN_SAMPLES) * len(STAGE2_METHODS) * len(STAGE2_EPSILONS)
                ...
                json.dumps(
                    {
                        ...
                        "expected_stage2_rows": expected_stage2_rows,
                        "note": "Stage 2 checkpoint incomplete; rerun script to continue.",
                    },
                    ensure_ascii=False,
                    indent=2,
                ),
