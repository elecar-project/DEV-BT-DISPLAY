...
OUTPUT_DIR = ROOT / "Result/M02_single/06.08_Test1-2(repl-new-tp-X)"
A04_EMBEDDINGS = ROOT / "Result/06.03_A04_min-test/A04-6(repl)_tok(para12-80)/embeddings_all-MiniLM-L6-v2.npy"
A04_EMBEDDINGS_META = ROOT / "Result/06.03_A04_min-test/A04-6(repl)_tok(para12-80)/embeddings_all-MiniLM-L6-v2.meta.json"
...
RUN_BASENAME = "06.08_Test1-2(repl-new-tp-X)"


CONFIGS = [
    {
        "selection_label": "06.08_Test1-2(repl-new-tp-X)",
        "report_name": f"{RUN_BASENAME}.md",
        "selection_reason": "使用者指定 repl-new-tp-X 單組參數，並以 BERTopic nr_topics='auto' 做 c-TF-IDF topic reduction。",
...
def get_embeddings(docs: list[str]) -> tuple[SentenceTransformer, np.ndarray, str]:
    ...
    if A04_EMBEDDINGS.exists() and A04_EMBEDDINGS_META.exists():
        ...
        if old_meta == current_meta:
            ...
            return model, np.load(EMBEDDINGS_PATH), f"重用 A04-6(repl) embeddings 快取並複製到 {RUN_BASENAME}。"
...
def export_representation(
    ...
    representation_stopwords: list[str],
    ...
    try:
        ...
        filter_topic_representations(topic_model, representation_stopwords)
        ...
        hit_summary = stopword_hit_summary(words, representation_stopwords)
        return {
            "status": "ok",
            "info": info,
            "words": words,
...
def build_combined_representation_table(
    ...
    combined.to_csv(out_dir / f"{RUN_BASENAME}_combined_representations.csv", index=False)
    combined.to_csv(out_dir / f"{RUN_BASENAME}_LLM.csv", index=False)
    combined_with_docs.to_csv(out_dir / f"{RUN_BASENAME}_combined_representations_with_docs.csv", index=False)
    md = [
    ...
    (out_dir / f"{RUN_BASENAME}_combined_representations.md").write_text("\n".join(md), encoding="utf-8")
    return combined
...
def llm50_prompt(topic_id: int, keywords: str, snippets: list[str]) -> list[dict[str, str]]:
...
def call_openrouter_label(
    ...
    response = client.chat.completions.create(
        model=LLM_MODEL_NAME,
        messages=llm50_prompt(topic_id, keywords, snippets),
        temperature=temperature,
        max_tokens=64,
        extra_headers={
            "HTTP-Referer": "https://local.bertopic.m02",
            "X-OpenRouter-Title": "BERTopic M02 LLM50 Validation",
        },
    )
    if not getattr(response, "choices", None):
...
def load_llm50_cache(cache_path: Path) -> dict[tuple[int, int], dict[str, Any]]:
...
def run_llm50_validation(
    ...
    if not api_key:
        ...
        errors.append({"representation_model": "llm50", "error": reason})
        ...
        pd.DataFrame(columns=["topic", "run", "model", "keywords", "label", "normalized_label", "error"]).to_csv(
            out_dir / f"{RUN_BASENAME}_LLM_detail.csv",
            index=False,
        )
        ...
        return {"status": "skipped", "topics": 0, "calls": 0, "stable_topics": 0, "error": reason}
    preflight_error = openrouter_preflight(api_key)
    if preflight_error:
        if existing_validation.exists():
            existing_df = pd.read_csv(existing_validation)
            if "successful_runs" in existing_df.columns and len(existing_df) > 1:
                ...
                return {
                    "status": "preflight_failed_existing_validation_preserved",
                    "topics": len(existing_df),
                    "calls": int(existing_df["successful_runs"].sum()),
                    "stable_topics": int(existing_df["stable"].astype(bool).sum())
                    if "stable" in existing_df.columns
    ...
    print(
        f"  LLM50 cache: {len(records)} done, {len(pending)} pending, workers={max_workers}",
        flush=True,
    )
    completed = 0
...
def write_llm_result_files(combined: pd.DataFrame, validation: pd.DataFrame | None, out_dir: Path) -> None:
    ...
    stable_text = "0 / 0"
    if isinstance(validation, pd.DataFrame) and not validation.empty and "stable" in validation.columns:
    ...
    report = [
        ...
        "",
        md_table(combined_rows, columns) if combined_rows else "無",
        "",
        "## LLM50 Validation Summary",
        "",
        md_table(validation_rows, list(validation_rows[0].keys())) if validation_rows else "無",
        "",
        "## Interpretation",
        "",
        "若 `stable=True`，代表同一組 keywords 與文本重跑時，模型給出的主題名稱在字面或詞彙組成上大致一致；若為 `False`，建議人工檢查該 topic 的 keywords 是否過於混雜。",
...
def write_report(
    ...
    if OPENROUTER_PREFLIGHT_ERROR:
        llm50_lines = [
            f"- status: `skipped`",
            f"- reason: {OPENROUTER_PREFLIGHT_ERROR}",
            "- BERTopic clustering and KeyBERT/POS/MMR representation outputs were still generated.",
            f"- skipped 狀態輸出：`{RUN_BASENAME}_LLM_validation.md`、`{RUN_BASENAME}_LLM_validation.csv`。",
        ]
    else:
        llm50_lines = [
            f"- 每個非 noise topic 使用 `{LLM_MODEL_NAME}` 對相同 keywords 與 {REPRESENTATIVE_DOCS_PER_TOPIC} 個代表原句重跑 {LLM50_RUNS} 次。",
            "- 穩定判定：normalized label 眾數比例 >= 0.70，或平均 token Jaccard similarity >= 0.65。",
            f"- 整合主表輸出：`{RUN_BASENAME}_LLM.md`、`{RUN_BASENAME}_LLM.csv`。",
            f"- 驗證統計輸出：`{RUN_BASENAME}_LLM_validation.md`、`{RUN_BASENAME}_LLM_validation.csv`。",
        ]
    report = [
        f"# {RUN_BASENAME} BERTopic + A05-8.4(human) stopwords + c-TF-IDF reduction - {label}",
        "",
        "## Run Info",
        "",
...
def run_one(
    ...
    llm50_summary = {"status": "not_run", "topics": 0, "calls": 0, "stable_topics": 0}
    if isinstance(default_words, pd.DataFrame):
        print("  LLM50 validation: openai/gpt-5.5", flush=True)
        llm50_summary = run_llm50_validation(docs, topics, default_words, OUTPUT_DIR, errors)
    llm50_df_path = OUTPUT_DIR / f"{RUN_BASENAME}_LLM_validation.csv"
    llm50_df = pd.read_csv(llm50_df_path) if llm50_df_path.exists() else None
    ...
    write_llm_result_files(combined_representations, llm50_df, OUTPUT_DIR)
    ...
    csv_paths = {
        ...
        "llm50_summary": f"{RUN_BASENAME}_LLM_validation.csv",
        "llm50_report": f"{RUN_BASENAME}_LLM_validation.md",
        "llm50_detail": f"{RUN_BASENAME}_LLM_detail.csv",
    }
    ...
    return {
        ...
        **metrics,
        "llm50_status": llm50_summary.get("status"),
        "llm50_topics": llm50_summary.get("topics"),
        "llm50_stable_topics": llm50_summary.get("stable_topics"),
